...
Long Short-Term Memory (LSTM) model
  - Architecture : LSTM(64) → Dropout → Linear(32) → Linear(1)
...
def run(data: dict, results_dir: str) -> dict:
    """Train, evaluate, and plot LSTM. Returns {name: metrics}."""
    ...
    model, val_losses, n_epochs = train_torch(
        _LSTMModel(data["n_features"]), Xs_tv, ys_tv, Xs_vv, ys_vv
    )
...
def _plot(y_true, y_pred, val_losses, metrics, results_dir):
    ...
    plt.tight_layout()
    path = os.path.join(results_dir, "lstm.png")
